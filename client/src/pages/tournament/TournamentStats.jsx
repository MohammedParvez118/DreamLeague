...
const TournamentStats = () => {
    ...
    const [sortBy, setSortBy] = useState('fantasy_points'); // fantasy_points, runs, wickets, etc.
    ...
    const fetchTournamentStats = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                setError('Failed to load tournament stats');
            }
        } catch (err) {
            console.error('Error fetching tournament stats:', err);
            setError(err.response?.data?.message || 'Failed to load tournament stats');
        } finally {
            setLoading(false);
        }
    };
    ...
    const calculateFantasyPoints = (player) => {
        ...
        if (player.total_runs > 0) {
            ...
            // Boundary bonuses (runs already counted above, these are ADDITIONAL bonuses)
            points += player.total_fours * 4; // +4 bonus for each 4 (total: +8 per four)
            points += player.total_sixes * 6; // +6 bonus for each 6 (total: +12 per six)

            // Milestone bonuses (only highest milestone)
            if (player.total_runs >= 100) {
                points += 16;
            } else if (player.total_runs >= 75) {
                points += 12;
            } else if (player.total_runs >= 50) {
                points += 8;
            } else if (player.total_runs >= 25) {
                points += 4;
            }
        ...
        if (player.total_wickets > 0) {
            ...
            // LBW/Bowled bonus (+8 for each)
            if (player.lbw_bowled_wickets > 0) {
                points += player.lbw_bowled_wickets * 8;
            }

            // Wicket milestones (per innings, so we use max_wickets_in_innings)
            if (player.max_wickets_in_innings >= 5) {
                points += 12;
            } else if (player.max_wickets_in_innings >= 4) {
                points += 8;
            } else if (player.max_wickets_in_innings >= 3) {
                points += 4;
            }
        ...
        if (player.total_catches > 0) {
            ...
            // 3-catch bonus (once for 3+ catches)
            if (player.total_catches >= 3) {
                points += 4;
            }
        }
        ...
        if (player.matches_in_xi > 0) {
            points += player.matches_in_xi * 4; // +4 for each match in playing XI
        }
        ...
        <div className="stats-container">
            <div className="stats-header">
                <div className="header-top">
                    ...
                    <button onClick={fetchTournamentStats} className="btn-refresh">
                ...
                <div className="stats-info">
                    <p>
                        ...
                        Showing stats from {stats.length} players across all matches with scorecards
                    </p>
                </div>
            </div>
                            ...
                            <tbody>
                                {filteredStats.map((player, index) => (
                                    <tr 
                                        ...
                                        className={index < 3 ? 'top-performer clickable-row' : 'clickable-row'}
                ...
                {activeTab === 'batting' && (
                    <div className="stats-table-container">
                        <table className="stats-table">
                            ...
                            <tbody>
                                {filteredStats.map((player, index) => (
                                    <tr key={player.player_id} className={index < 3 ? 'top-performer' : ''}>
